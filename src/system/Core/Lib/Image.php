...
class Image
{
	...
	public static function watermark($srcfile, $markpic, $markpos = 6, $marktrans = 100)
	{
		...
		switch ($wmType) {
			case 1 : 
				$wm = imagecreatefromgif($markpic); break;
			case 2 :
				$wm = imagecreatefromjpeg($markpic); break;
			case 3 :
				$wm = imagecreatefrompng($markpic); break;
		}
		if (!$wm) return false;
		...
		switch ($markpos) {				
			...
			case 3 :  //顶部居右
				$srcX = $srcW - $wmW; $srcY = 0; break;
			case 4 :  //底部居左
				$srcX = 0; $srcY = $srcH - $wmH; break;				
			case 5 :  //底部居中
				$srcX = ($srcW - $wmW) / 2; $srcY = $srcH - $wmH; break;		
			case 6 :  //底部居右
	...
	public static function watermarkText($srcfile, $fontfile, $text, $size, $rgb, $markpos = 6, $options = array())
	{
		...
		switch ($srcType) {
			case 1 : 
				$image = imagecreatefromgif($srcfile); break;
			case 2 :
				$image = imagecreatefromjpeg($srcfile); break;
			case 3 :
				$image = imagecreatefrompng($srcfile); break;
		} 
		...
		switch ($markpos) {				
			case 1 :  //顶部居左
				$srcX = $offset; $srcY = $textH + $offset; break;				
			case 2 :  //顶部居中
				$srcX = ($srcW - $textW) / 2; $srcY = $textH + $offset; break;				
			case 3 :  //顶部居右
				$srcX = $srcW - $textW - $offset; $srcY = $textH + $offset; break;
			case 4 :  //底部居左
				$srcX = $offset; $srcY = $srcH - $offset; break;				
			case 5 :  //底部居中
				$srcX = ($srcW - $textW) / 2; $srcY = $srcH-$offset; break;		
			case 6 :  //底部居右
				$srcX = $srcW-$textW-$offset; $srcY = $srcH-$offset; break;
			default : //随机
				$srcX = mt_rand(0,$srcW-$textW); $srcY = mt_rand(0,$srcH-$textH);
		}
		...
		switch ($srcType) {
			case 1 :
				imagegif($image, $srcfile); break;
			case 3 : 
				imagepng($image, $srcfile); break;
			default :
